function checkCode(pre_code) {
  ...
  let index = pre_code_data.indexOf(pre_code);
  if (index != -1) {
    const room = reservation_application_data[index][2];
    const date = reservation_application_data[index][4].slice(0, 10);
    deletePreReservation(room, date);
  }
}


function deletePreReservation(room, date) {
